...
class AcquisitionPosition(PricedPosition[A, D, I | T, L], Generic[A, D, I, T, L]):
    ...
    def cost_basis_sold(self) -> Decimal:
        ...
        lot: AcquisitionLot
        return safe_sum(lot.cost_basis_sold for lot in self._lots_with_records_out)

    @property
    def proceeds(self) -> Decimal:
        # workaround for type checker
        lot: AcquisitionLot
        return safe_sum(lot.proceeds for lot in self._lots_with_records_out)

    @property
    def pnl(self) -> Decimal:
        lot: AcquisitionLot
        return safe_sum(lot.pnl for lot in self._lots_with_records_out)