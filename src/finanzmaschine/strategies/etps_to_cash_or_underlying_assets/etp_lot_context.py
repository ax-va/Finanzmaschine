...
class ShareLotContext:
    ...
    def __init__(
        ...
        underlying_local_high: Decimal | None = None,
        underlying_vac_upper_bound: Decimal | None = None,
        underlying_acc_upper_bound: Decimal | None = None,
    ):
        ...
        if underlying_vac_upper_bound is None:
            if self.underlying_local_high is not None and self.underlying_loss_pct is not None:
                underlying_vac_upper_bound = self.underlying_local_high * Decimal(
                    str(1 - self.underlying_loss_pct)
                )
        ...
        if underlying_acc_upper_bound is None:
            if (
                ...
                underlying_acc_upper_bound = self.underlying_vac_upper_bound / Decimal(
                    str(
                        math.sqrt(
    ...
    @property
    def underlying_take_profit_price(self) -> Decimal | None:
        if self.underlying_profit_pct is not None:
            underlying_price_in = self.etp_lot.record_in.price / Decimal(str(self.etp_lot.record_in.entitlement))
            return underlying_price_in * Decimal(str(1 + self.underlying_profit_pct))
        else:
            return None

    @property
    def underlying_stop_loss_price(self) -> Decimal | None:
        if self.underlying_loss_pct is not None:
            underlying_price_in = self.etp_lot.record_in.price / Decimal(str(self.etp_lot.record_in.entitlement))
            underlying_stop_loss: Decimal = underlying_price_in * Decimal(str(1 - self.underlying_loss_pct))
