...
def test_close_position(
    ...
    for lot in position.lots_with_records_sold:
        ...
        for record in lot.records_sold:
            assert record.quantity == Decimal(golden_values.row(i, named=True)["quantity_closed"])
            assert record.fee == Decimal(golden_values.row(i, named=True)["fee_closed"])

            record_proceeds = round_to_quantum(
                record.quantity * record.price - record.fee,
                lot.record_in.quote_asset.quantum,
            )
            assert record_proceeds == Decimal(golden_values.row(i, named=True)["proceeds"])

            record_cost_basis_sold = round_to_quantum(
                record.quantity / lot.record_in.quantity * lot.cost_basis,
                lot.record_in.quote_asset.quantum,
            )
            assert record_cost_basis_sold == Decimal(golden_values.row(i, named=True)["cost_basis_sold"])
